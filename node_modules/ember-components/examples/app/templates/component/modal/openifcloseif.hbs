...
<p>You've seen how a modal is opened/closed in the {{#link-to 'component.modal.simple'}}simple page{{/link-to}} by the <i>\{{em-modal-toggler}}</i> tag, But sometimes it makes sense to programatically open or close a modal, for this, bind the <i>open-if</i> property to a controller property, once the property value is set to <i>true</i> the modal will be opened automatically.</p>
